...
export class CssParser {
  ...
  private static mapCssPropertyToFeatureId(property: string): string | null {
    ...
    if (!normalizedProperty || /\s/.test(normalizedProperty)) return null;
    ...
    const nonBaselineProperties = new Set([
      'float', // Legacy layout - encourage flexbox/grid
      'clear', // Related to float
      'container-type', // Container queries - newer feature
      'aspect-ratio', // Newer feature
      'scroll-behavior', // Newer feature  
      'scroll-snap-type', // Newer feature
      'overscroll-behavior', // Newer feature
      'backdrop-filter', // Newer feature
      'view-transition-name', // Very new feature
      'touch-action', // May not be baseline
