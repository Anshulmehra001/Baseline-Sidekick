 ...
 */
export class BaselineScoreManager {
  private static instance: BaselineScoreManager;
  private statusBarItem: vscode.StatusBarItem;
  private scoreHistory: ScoreHistory[] = [];

  public static getInstance(): BaselineScoreManager {
   ...
   */
  calculateFileScore(fileUri: vscode.Uri, features: FeatureAnalysis): FileScore {
   ...
   */
  async calculateWorkspaceScore(fileScores: FileScore[]): Promise<WorkspaceScore> {
   ...
   */
  private updateStatusBar(workspaceScore: WorkspaceScore): void {
  ...
  private generateScoreImprovementSuggestions(features: FeatureAnalysis): string[] {
  ...
  private calculateModernizationTrend(): 'improving' | 'stable' | 'declining' {
  ...
  private calculateAchievement(score: number, fileCount: number): Achievement {
  ...
  private generateWorkspaceRecommendations(fileScores: FileScore[]): string[] {
  ...
  private updateScoreHistory(workspaceScore: WorkspaceScore): void {
  ...
  private createEmptyWorkspaceScore(): WorkspaceScore {
    return {
      overallScore: 100,
      grade: 'A+',
      totalFiles: 0,
  ...
  private async generateScoreWebviewContent(): Promise<string> {
...
export interface FeatureAnalysis {
  ...
  nonBaseline: NonBaselineFeature[];
}
...
export interface NonBaselineFeature {
  name: string;
  type: 'css' | 'html' | 'javascript';
...
export interface WorkspaceScore {
  ...
  modernizationTrend: 'improving' | 'stable' | 'declining';
